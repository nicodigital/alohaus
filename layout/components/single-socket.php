<div class="socket fixed left-0 bottom-0 w-full ctr-px">
  <div class="container">
    <div class="flex justify-between items-center py-1 border-t border-w">

      <div class="grid grid-cols-2 xg:w-50% items-center">
        <div class="text-h1 font-title">
          Volta
        </div>
        <p class="translate-y-[.7rem]">
          Packaging
        </p>
      </div>

      <div class="togg-col pointer-1x">
        <svg width="33" height="33" viewBox="0 0 33 33" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path fill-rule="evenodd" clip-rule="evenodd"
            d="M4.64074 32.7103L26.5665 10.7845L26.5666 32.8686L32.4762 32.8687V0.696201H0.303711V6.6058H22.3878L0.462025 28.5316L4.64074 32.7103Z"
            fill="currentColor" />
        </svg>
      </div>
      
    </div>
  </div>
</div>